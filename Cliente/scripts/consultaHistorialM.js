...
async function loadHistorial(user) {
  ...
  try {
    const response = await fetch(
      `http://${HOST}:4000/buscarHistorial?user=${user}`
    );
    ...
    document.getElementById("inputFNacimiento").value =
      data.result.fechaNaciemiento;
    document.getElementById("inputLNacimiento").value = data.result.lugarOrigen;
    document.getElementById("inputNacionalidad").value =
      data.result.nacionalidad;
    document.getElementById("inputDireccion").value =
      data.result.lugarResidencia;
    document.getElementById("inputNombreResponsble").value =
      data.result.personaResponsable;
    document.getElementById("inputVinculo").value =
      data.result.prVinculoconPaciente;
    document.getElementById("txtHeredo").value =
      data.result.anteHeredoFamiliares;
    document.getElementById("txtPersonalesNP").value =
      data.result.antePersonalesNP;
    document.getElementById("txtPersonalesP").value =
      data.result.antePersonalesP;
